...
**It overwrites `constant/polyMesh`**, which is gitignored for this case. Restore
the Pointwise mesh from its tag:
...
## Which `constant/polyMesh` is tracked, and which is not

**Exactly one case is ignored — this one.** `cases/3d-inflow/constant/polyMesh` is
generated by `./Allmesh` from `case.yaml`, and tracking a generated artifact of
that size was costing real friction: each regeneration rewrote roughly 350 000
tracked lines and left the tree dirty enough to block `git checkout`.

**Every other case stays tracked, and must.** Their meshes came out of Pointwise
V18.5R2 — a licensed, interactive tool whose `.pw` project files are binary — and
none of them has a generator. For those 41 case directories the committed
`polyMesh` is the only copy in existence. The `.gitignore` rule is written as a
full path (`cases/3d-inflow/constant/polyMesh/`) precisely so it cannot generalise;
**do not** rewrite it as `**/constant/polyMesh/`.

3d-inflow's own Pointwise export — 99 240 tetrahedra, 2044 triangular inflow
faces, the substrate the regression golden is bound to — is preserved on a tag
rather than in `HEAD`:
...
* A plain `git checkout -- constant/polyMesh` **no longer restores anything** for
  this case. Git cannot check out a path it does not track; the command succeeds
  silently and changes nothing. Use the tag.
* A fresh clone has **no mesh** for 3d-inflow. `./Allrun` says so and points at
  both remedies; `tests/regression` skips rather than erroring.

(A historical note, since the old rules are still visible in the file's history:
rules of the form `constant/polyMesh/*` never worked, because a pattern containing
a slash anchors to the repository root and so matched nothing under `cases/*/` —
finding RP-15. That anchoring is exactly what makes the current single-case rule
correct.)

The generated `system/blockMeshDict`, `snappyHexMeshDict` and `meshQualityDict`
are gitignored for the same reason: they are derived from `case.yaml`, which is
