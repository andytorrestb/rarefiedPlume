...
def f(x):
    return np.sin(x) * angularDependence(1.4, x)
...
def calculateRhoN(face_data, physical_props):
    ...
    print('vel_l', vel_l)
    Po = physical_props['Po'] * 6894.75729
    # Po = 475*6894.75729
    print('Po', Po)
    pi = math.pi
...
def calculateU(face_data, physical_props):
    ...
    Ux = np.abs(v_l * np.cos(theta))
    Uy = np.abs(v_l * np.sin(theta))
    Uz = 0.0
...
def plumeSourceFlowModel():
  ...
  # ======================================= #
    ...
    physical_props['Po'] = 100 # 5psi in Pa
